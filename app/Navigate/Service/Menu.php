...
class Navigate_Service_Menu extends Zoo_Service {
	...
	function getContentMenu() {
	    if (!$this->container) {
            ...
            if (!$this->container) {
        	    ...
        	    $items = Zoo::getService('content')->getContent(array('group' => 'category', 
        	    													'active' => true,
        	                                                        'order' => 'published', 
        	    													'render' => false), 0, 0);
        		...
        		$tags = array('menu');
                try {
                    Zoo::getService('cache')->save($this->container,
                                                   $cacheid,
                                                   $tags);
                }
                catch (Zoo_Exception_Service $e) {
	 ...
	 */
	function getBreadcrumbsFromNode($node, &$array = array()) {
	    $array[] = $this->nodeToPage($node, true);
	    if ($parent = $node->getParent()) {
	        return $this->getBreadcrumbsFromNode($parent, $array);
	    }
	    else {
	        array_reverse($array);
	        $i = 0;
	...
	function nodeToPage(Zoo_Content_Interface $node, $active = false) {
	    return new Zend_Navigation_Page_Uri(array('uri' => $node->url(),
    	                                          'title' => $node->title,
    	                                          'label' => $node->title,
	                                              'id' => 'node_'.$node->id,
	...
	protected function getRootPage() {
	    return new Zend_Navigation_Page_Uri(array('uri' => '/',
